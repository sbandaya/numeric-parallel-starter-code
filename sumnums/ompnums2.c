...
int main (int argc, char *argv[])
{
   ...
   //info for each thread following pthread implentation
   unsigned long long gsum[NUM_THREADS];
   unsigned long long gstart[NUM_THREADS];
   unsigned long long gend[NUM_THREADS];
   ...
   //splits work evenly among threads makes sure each thread has individual i to avoid clashes
   #pragma omp parallel for num_threads(NUM_THREADS) reduction(+:sum) private(i)
   for(i=0; i<=COUNT; i++)
   {
         int idx = omp_get_thread_num();
         sum+=i;
         
        //verifies sums and ranges of each thread 
         gsum[idx] +=i;
         if(gsum[idx] == i)
         {
            gstart[idx] = i;
   ...
   ////verifies sums and ranges of each thread 
   for (unsigned long long t = 0; t < NUM_THREADS; t++) 
   {
      printf("Thread %llu summing range %llu to %llu, gsum=%llu\n",t, gstart[t], gend[t], gsum[t]);
   }
